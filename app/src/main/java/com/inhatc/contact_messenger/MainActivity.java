...
public class MainActivity extends AppCompatActivity
            ...
            public void onClick(View view) {
                ...
                if (checkBox.isChecked()){
                    SafetyNet.SafetyNetApi.verifyWithRecaptcha(googleApiClient,SiteKey)
                            .setResultCallback(new ResultCallback<SafetyNetApi.
                                    RecaptchaTokenResult>() {
                                @Override
                                public void onResult(@NonNull SafetyNetApi.RecaptchaTokenResult
                                                             recaptchaTokenResult) {
    ...
    public void onClick(View v){
        if(v==btnLogin){
            ...
            if(!captchaBool) {
                Toast.makeText(getApplicationContext(), "캡챠 인증을 해주세요.",
                        Toast.LENGTH_SHORT).show();
            }else {
                if (strID.equals("") || strPWD.equals("")) {
                    Toast.makeText(getApplicationContext(), "로그인 정보를 입력해 주세요.",
                            Toast.LENGTH_SHORT).show();
                } else {
                    myDB_Reference.child(strHeader).child(strID).
                        ...
                        public void onDataChange(@NonNull DataSnapshot snapshot) {
                            ...
                            if (value != null) {
                                if (value.ID.equals(strID) && value.PW.equals(strPWD)) {
                                    Intent contentMain = new Intent(MainActivity.this,
                                            MainContact.class);
                                    //액티비티를 통해 계정 정보 전달
                                    contentMain.putExtra("myInfo", value.ID);
                                    startActivity(contentMain);
                                    Toast.makeText(getApplicationContext(), value.Name +
                                            "님 어서오세요", Toast.LENGTH_SHORT).show();
                                    finish(); //로그인 성공하면 액티비티 종료
                                } else {
                                    Toast.makeText(getApplicationContext(),
                                            "로그인 정보를 확인해 주세요", Toast.LENGTH_SHORT).show();
                                }
                            } else {
                                Toast.makeText(getApplicationContext(),
                                        "로그인 정보를 확인해 주세요", Toast.LENGTH_SHORT).show();
